...
from processing import threshold_processing, auto_selection, cut, rotation,smooth, jpeg,CVZ, alpha1
...
if __name__ == '__main__':
    ...
    plot_results(np.arange(0, 100, 1), false_detection_proximity_array, 'Figure 1', 'X-axis', 'Y-axis', "red")
    ...
    for cut_param in cut_param_array:
        cut_p.append(cut(cut_param,reverse_array,image_array,phase_array,abs_spectre))

    rotation_param_array = np.arange(1, 90, 8.9)
    rotation_p = []
    for rotation_param in rotation_param_array:
        rotation_p.append(rotation(rotation_param, reverse_image,phase_array,abs_spectre))

    smooth_param_array = np.arange(3, 15, 2)
    smooth_p = []
    for smooth_param in smooth_param_array:
        smooth_p.append(smooth(smooth_param,reverse_image,phase_array,abs_spectre))

    jpeg_param_array = np.arange(30, 91, 10)
    jpeg_p = []
    for jpeg_param in jpeg_param_array:
        jpeg_p.append(jpeg(int(jpeg_param),reverse_image, phase_array, abs_spectre))

    plot_results(cut_param_array, cut_p, 'CUT', 'X-axis', 'Y-axis', "red")
    plot_results(rotation_param_array, rotation_p, 'ROTATION', 'X-axis', 'Y-axis', "red")
    plot_results(smooth_param_array, smooth_p, 'SMOOTHING PARAMETERS', 'Window Size', 'Proximity Measure', "blue")
    plot_results(jpeg_param_array, jpeg_p, 'JPEG QUALITY FACTOR', 'Quality Factor', 'Proximity Measure', "green")
